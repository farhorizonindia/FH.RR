...
public partial class response : System.Web.UI.Page
{
    ...
    private void calculateall(DataTable dt)
    {
        ...
        lblGetGrandTotal.Text = "INR " + TaxableAmount.ToString("##,0");
        lblgetTotal.Text = "INR " + gross.ToString("##,0");

        Label2.Text = "Taxable Amount";
        if (discount != 0)
        {
    ...
    public void SendCancelledMail()
    {
        try
        {
            ...
            #region Email Body
            StringBuilder sb = new StringBuilder();
            sb.Append("<div>Booking No : " + dtbkcode.Rows[0].ItemArray[0].ToString() + "</div>  <div> Date of Booking : " + Convert.ToDateTime(System.DateTime.Now).ToString("d MMMM, yyyy") + " </div> <div><br/> </div> <div> Dear " + Session["InvName"].ToString() + ",</div> <div><br/></div><div> Namaskar! Greetings from ARC Adventure Resorts & Cruises Pvt. Ltd.!</div> <div><br/> </div><div> Thank you for showing interest in Mahabaabu. <br /> You were trying to make a booking from " + lblArrvDate.Text + " to " + lblDepartDate.Text + " but no payment was processed against this booking. </div><div><br/></div> ");
    ...
    private void ShowHotelBookingDetails(BALBooking booking)
    {
        try
        {
            ...
            if (Bookingdt != null && Bookingdt.Rows.Count > 0)
            {
                ...
                {
                    Bookingdt.Rows[i]["Price"] = Bookingdt.Rows[i]["Price"].ToString().Split('R')[1];
                    Bookingdt.Rows[i]["Inclusivetax"] = Bookingdt.Rows[i]["Inclusivetax"].ToString().Split('R')[1];
                    tax = tax + Convert.ToDouble(Bookingdt.Rows[i]["Tax"].ToString().Split('R')[1]);
                }
        ...
        try
        {
            ...
            if (Output1 == Output)
            {
                ...
                else
                {
                    ...
                    lblGetGrandTotal.Text = "INR " + Convert.ToInt32((totalAmount1 - totdiscount).ToString()).ToString("##,0");
                    //gdvCruiseRooms.FooterRow.Cells[2].Text = "Total <br> Service Tax @ 4.50% <br> <b> Grand Total </b>";
                    Label1.Text = Session["gettax"].ToString().Split('R')[1] + "%";
                    string amountText = string.Format("{0}<br>{1}<br>{2}",
                    ...
                    double balanceAmount = Math.Round((Convert.ToDouble(lblGross.Text.Split('R')[1].Replace(",", "")) - Convert.ToDouble(lblTotPaid.Text.Replace(",", ""))));
                    if (balanceAmount > 0)
                    {
                        lblBalance.Text = Math.Round((Convert.ToDouble(lblGross.Text.Split('R')[1].Replace(",", "")) - Convert.ToDouble(lblTotPaid.Text.Replace(",", "")))).ToString("#.##");
                        lblBalance.Text = Convert.ToInt32(lblBalance.Text).ToString("##,0");
    ...
    public void SendCruiseMailfor25()
    {
        try
        {
            ...
            #region Email Body
            StringBuilder sb = new StringBuilder();
            sb.Append("<div>Booking No : " + dtbkcode.Rows[0].ItemArray[0].ToString() + "</div>  <div> Date of Booking : " + Convert.ToDateTime(System.DateTime.Now).ToString("d MMMM, yyyy") + " </div> <div><br/> </div> <div> Dear " + Session["InvName"].ToString() + ",</div> <div><br/></div><div> Namaskar! Greetings from ARC Adventure Resorts & Cruises Pvt. Ltd.!</div> <div><br/> </div><div> Thank you for booking " + packageName + " from " + lblArrvDate.Text + " to " + lblDepartDate.Text + ". We are in receipt of 25% Payment amounting to Indian Rupees " + Session["getPaid"].ToString() + "</div><div><br/></div> ");
    ...
    public void SendCruiseMail()
    {
        try
        {
            ...
            StringBuilder sb = new StringBuilder();
            sb.Append("<div>");
            sb.Append("<div>Booking No:" + dtbkcode.Rows[0].ItemArray[0].ToString() + "</div>  <div> Date of Booking: " + Convert.ToDateTime(System.DateTime.Now).ToString("d MMMM, yyyy") + " </div> <div><br/> </div> <div> Dear " + Session["InvName"].ToString() + ",</div> <div><br/></div><div> Namaskar! Greetings from ARC Adventure Resorts & Cruises!</div> <div><br/> </div><div> Thank you for choosing MV Mahabaahu Cruise, River Brahmaputra for your vacation.<br/>" + packageName + " on MV Mahabaahu.</div><div><div> <div><br/></div><div>Check In: " + lblArrvDate.Text + "</div><div><br/></div><div>Check Out: " + lblDepartDate.Text + "</div><div><br/></div> ");
