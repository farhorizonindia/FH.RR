...
public partial class Cruise_Booking_SummarizedDetails : BaseClass
{
    ...
    protected void LinkButton1_Click(object sender, EventArgs e)
    {
        ...
        {
            string agntid = Request.QueryString["agentid"];
            Response.Redirect("SearchProperty.aspx?agentid=" + agntid + "");
        }
        else
        {
            Response.Redirect("SearchProperty.aspx");
        ...
        try
        {
            ...
            if (dt != null && dt.Rows.Count > 0)
            {
                ...
                if(ddlCurrency.Text != "USD")
                {
                    ...
                    lblnetAmount.Text = "USD " + ViewState["Comman"];

                    Session["NetAmounts"] = "USD " + ViewState["Comman"];
                ...
                else
                {
                    ...
                    Session["NetAmounts"] = "INR " + Totamt.ToString("##,0");
                }
    ...
    public void sendMail2()
    {
        if (Session["Phonecheck"] == null)
        {
            ...
            //    string ccEmail = ConfigurationManager.AppSettings["ccEmail"];
                ...
                SmtpServer.Send(mail);
                ScriptManager.RegisterStartupScript(this, this.GetType(), "Showstatus", "javascript:alert('To validate your email id we have sent a code on your email, please key in the same to validate it.')", true);
                custRegis.Visible = false;
                //customerLogin.Visible = true;
        ...
        try
        {
            ...
            if (lblDiscountper.Text != "0%")
            {
                if (ddlCurrency.Text != "USD")
                {
                    ...
                    lblDiscount.Text = "USD" + " " + ViewState["discountamount"];
                }
                else
                {
                    lblDiscount.Text = dtGetBookedRooms.Rows[0]["Currency"].ToString() + " " + totyadiscount.ToString("##,0");
            ...
            if (ddlCurrency.Text != "USD")
            {
                currency(total.ToString("##,0"), ddlCurrency.Text);
                lbltotAmt.Text = "USD " + " " + ViewState["totalamount"];
                currency(taxableamt.ToString("##,0"), ddlCurrency.Text);
                lbltaxin.Text = "USD " + ViewState["taxablepamount"];
                currency(Convert.ToDouble(gettax.ToString()).ToString("##,0"), ddlCurrency.Text);
                lblTax.Text = "USD " + ViewState["gst"];

                currency(getalltotal.ToString("##,0"), ddlCurrency.Text);
                lblalltotal.Text = "USD " + ViewState["ttl"];

                currency(Convert.ToDouble(grosstotal.ToString()).ToString("##,0"), ddlCurrency.Text);
                lblGross.Text = "USD " + ViewState["gross"];
            }
            else
            {
                lblGross.Text = dtGetBookedRooms.Rows[0]["Currency"].ToString() + " " + Convert.ToDouble(grosstotal.ToString()).ToString("##,0");
            ...
            if (Convert.ToDateTime(Request.QueryString["CheckinDate"]).AddDays(-90) < System.DateTime.Now)
            {
                if (ddlCurrency.Text != "USD")
                {
                    currency(Convert.ToDouble(Math.Round(((100 * TotalPaybleAmt) / 100)).ToString("#.##")).ToString("##,0"), ddlCurrency.Text);
                    txtPaidAmt.Text = ViewState["paidamnt"].ToString();
                }
                else
                {
                    txtPaidAmt.Text = Convert.ToDouble(Math.Round(((100 * TotalPaybleAmt) / 100)).ToString("#.##")).ToString("##,0");
            ...
            if (ddlCurrency.Text != "USD")
            {
                ...
                lblBalanceAmt.Text = "USD "+ ViewState["balanceamnt"].ToString();
            }else
            {
                lblBalanceAmt.Text = dtGetBookedRooms.Rows[0]["Currency"].ToString() + " " + Convert.ToDouble(Math.Round((TotalPaybleAmt - Convert.ToDecimal(txtPaidAmt.Text))).ToString()).ToString("##,0");
            }
        ...
        try
        {
            ...
            else
            {
                if (Session["CustId"] != null && Session["UserCode"] == null)
                {
                    ...
                    if (Session["AgentId"] != null)
                    {
                        blsr._iAgentId = Convert.ToInt32(Session["AgentId"]);
                    }
                    else
                    {
                        blsr._iAgentId = 247;
    ...
    public void sendMail_Proceed(string Name, string Email)
    {
        ...
        try
        {
            ...
            sb.Append("<div> &nbsp;Booking Amount: " + Session["NetAmounts"].ToString() + "</div>");
    ...
    protected void btnPayProceed_Click(object sender, EventArgs e)
    {
        ...
        try
        {
                ...
                if (Session["UserCode"] != null)
                {
                    ...
                    if (Debugger.IsAttached)
                    {
                        if (Convert.ToBoolean(Session["IsPaymentByPass"]) == true)
                        {
                            ...
                            checkagnturl();
                            if (Session["agnturi"] != null)
                            {
                                if (Session["SetCurrency"].ToString() != "USD")
                                {
                                    string sub = lblGross.Text.Substring(4);
                                    currency(sub, "INR");
                                    string gross = "USD " + ViewState["totalamount"].ToString();

                                    string ttlAmnt = lbltotAmt.Text.Substring(4);
                                    currency(ttlAmnt, "INR");
                                    string totalamnt = "USD " + ViewState["totalamount"].ToString();

                                    string taxbleAmnt = lbltaxin.Text.Substring(4);
                                    currency(ttlAmnt, "INR");
                                    string taxableAmnt = "USD " + ViewState["totalamount"].ToString();

                                    string gstamnt = lblTax.Text.Substring(4);
                                    currency(gstamnt, "INR");
                                    string gst = "USD " + ViewState["totalamount"].ToString();

                                    currency(txtPaidAmt.Text, "INR");
                                    string AdvanceAmt = "USD " + ViewState["totalamount"].ToString();

                                    string urlagnt = Session["agnturi"].ToString();
                                    urlagnt += "?PackageName=" + Session["packagenames"].ToString() + "&CheckInDate=" + Session["chkindate"].ToString() + "&CheckOutdate=" + Session["chkoutdate"].ToString() + "&InvoiceTo=" + lblAgentName.Text.ToString() + "&Gross=" + gross.ToString() + "&TotalAmt=" + totalamnt.ToString() + "&TaxableAmt=" + taxableAmnt.ToString() + "&GST=" + gst.ToString() + "&AdvanceAmt=" + AdvanceAmt.ToString() + "";

                                    Response.Redirect(urlagnt);
                                }
                                else
                                {
                                    string urlagnt = Session["agnturi"].ToString();
                                    urlagnt += "?PackageName=" + Session["packagenames"].ToString() + "&CheckInDate=" + Session["chkindate"].ToString() + "&CheckOutdate=" + Session["chkoutdate"].ToString() + "&InvoiceTo=" + lblAgentName.Text.ToString() + "&Gross=" + lblGross.Text.ToString() + "&TotalAmt=" + lbltotAmt.Text.ToString() + "&TaxableAmt=" + lbltaxin.Text.ToString() + "&GST=" + lblTax.Text.ToString() + "&AdvanceAmt=" + txtPaidAmt.Text.ToString() + "";
                        ...
                        if (Convert.ToBoolean(Session["IsPaymentByPass"]) == true)
                        {
                            ...
                            if (Session["agnturi"] != null)
                            {
                                if (Session["SetCurrency"].ToString() != "USD")
                                {
                                    ...
                                    string urlagnt1 = Session["agnturi"].ToString();
                                    urlagnt1 += "?PackageName=" + Session["packagenames"].ToString() + "&CheckInDate=" + Session["chkindate"].ToString() + "&CheckOutdate=" + Session["chkoutdate"].ToString() + "&InvoiceTo=" + lblAgentName.Text.ToString() + "&Gross=" + gross.ToString() + "&TotalAmt=" + totalamnt.ToString() + "&TaxableAmt=" + taxableAmnt.ToString() + "&GST=" + gst.ToString() + "&AdvanceAmt=" + AdvanceAmt.ToString() + "";

                                    Response.Redirect(urlagnt1);
                                }
                                else
                                {
                                    string urlagnt1 = Session["agnturi"].ToString();
                                    urlagnt1 += "?PackageName=" + Session["packagenames"].ToString() + "&CheckInDate=" + Session["chkindate"].ToString() + "&CheckOutdate=" + Session["chkoutdate"].ToString() + "&InvoiceTo=" + lblAgentName.Text.ToString() + "&Gross=" + lblGross.Text.ToString() + "&TotalAmt=" + lbltotAmt.Text.ToString() + "&TaxableAmt=" + lbltaxin.Text.ToString() + "&GST=" + lblTax.Text.ToString() + "&AdvanceAmt=" + txtPaidAmt.Text.ToString() + "";
                ...
                else
                {
                    ...
                    else
                    {
                        try
                        {
                            ...
                            string bkngno = dt.Rows[0]["BookingCode"].ToString();
                            string bkngdt = Convert.ToDateTime(dt.Rows[0]["BookingDate"].ToString()).ToString("d MMMM, yyyy");
                            sendMail_Proceed( FirstName, Email);
                        }
                        catch { }
                        if (Convert.ToBoolean(Session["IsPaymentByPass"]) == true)
            ...
            btnPayProceed.Attributes.Add("onclick", "return:javascript preventBack()");
    ...
    protected void btnguestverify_Click(object sender, EventArgs e)
    {

        try
        {
            if (txtguestverify.Text == hdfguestcode.Value)
            {
                if (Session["refrence"] != null)
                {
                    //ClientRegister1();
            ...
            if (Session["Dollar"].ToString() == "1")
            {
                ...
                foreach (var r in aa)
                {
                    ...
                    if (r.Name.Trim().ToLower() == sd.Trim().ToLower())
                    {
                        ...
                        ViewState["taxablepamount"] = value;
                        ViewState["gst"] = value;
                        ViewState["ttl"] = value;
                        ViewState["gross"] = value;
                        ViewState["paidamnt"] = value;
                        ViewState["balanceamnt"] = value;
